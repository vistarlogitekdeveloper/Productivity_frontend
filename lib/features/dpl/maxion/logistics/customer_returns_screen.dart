                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            ...
                            style: const TextStyle(fontSize: 12, color: DplColors.textSecondary),
                          ),
                        ],
                      ),
                    ),
  ...
  Future<void> _decide(String disposition) async {
    ...
    if (labels > 0) {
      await showDialog<void>(
        context: context,
        builder: (ctx) => AlertDialog(
          icon: const Icon(Icons.print_outlined, size: 36, color: DplColors.primary),
...
class DispositionPill extends StatelessWidget {
  ...
  factory DispositionPill.forLine(DplReturnLine l) => switch (l.disposition) {
        'restocked' => const DispositionPill(label: 'Restocked', color: DplColors.success),
        'scrapped' => const DispositionPill(label: 'Scrapped', color: DplColors.error),
        _ => const DispositionPill(label: 'Pending QA', color: DplColors.warning),
      };
      ...
      child: Row(
        children: [
                ...
                Row(
                  children: [
                    ...
                    Text(wheel ? 'Wheel' : 'Manual × ${l.qty}',
                        style: const TextStyle(fontSize: 11.5, color: DplColors.textSecondary)),
                  ],
                ),
          ...
          if (onRemove != null)
            IconButton(
              tooltip: 'Remove line',
              icon: const Icon(Icons.delete_outline, color: DplColors.error),
              onPressed: onRemove,
            )
          else
            const SizedBox(width: 6),
    ...
    return AlertDialog(
      ...
      content: SingleChildScrollView(
        child: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('For wheels that came back without one of our labels.',
            ...
            if (_searchError != null)
              Padding(
                ...
                child: Text(_searchError!, style: const TextStyle(fontSize: 12, color: DplColors.textSecondary)),
              ),
