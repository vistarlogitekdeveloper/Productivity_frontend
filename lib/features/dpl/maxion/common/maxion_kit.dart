...
class DplCountChip extends StatelessWidget {
  ...
  /// Null means the theme's primary. It cannot DEFAULT to that: the palette is
  /// resolved per theme now, so `DplColors.primary` is a getter and a default
  /// parameter value has to be a compile-time constant.
  final Color? color;

  const DplCountChip({super.key, required this.label, required this.count, this.color});
  ...
  Widget build(BuildContext context) {
    ...
    return Chip(
      ...
      label: Text('$label  $count', style: TextStyle(color: c, fontWeight: FontWeight.w600)),
    );
  }
}
